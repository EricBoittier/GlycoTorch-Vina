...
struct quasi_newton {
	...
	// clean up
	void operator()(model& m, const precalculate& p, const igrid& ig, output_type& out, change& g, const vec& v/*, std::vector< std::vector<int> > glyco_info*/, fl tmp, const fl chi_coeff, const fl chi_cutoff) const; // g must have correct size
};
